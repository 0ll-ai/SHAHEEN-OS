...
/**
 * Bot-thread identity used to route source events that originate outside a SHAHEENOS topic.
 */
...
export interface AgentSignalProducerScopeInput {
  ...
  taskId?: string;
  /** Optional SHAHEENOS topic identifier. Takes precedence over task and bot-thread metadata. */
  topicId?: string;
...
/**
 * Topic identity used when an AgentSignal chain is scoped to one SHAHEENOS topic.
 */
export interface AgentSignalTopicScopeKeyInput {
  /** Stable SHAHEENOS topic identifier. */
  topicId: string;
}
